	...
	class LG_List_Table extends WP_List_Table{
		...
		*/
		private function table_data()
		{
			...
			if ($resultat) {				
				foreach ($resultat as $key => $value) {		
					$option_name_view = '_lg_tool_button_view_counter_' . $value->id;
					$option_name_action = '_lg_tool_button_action_counter_' . $value->id;
					...
					$title = !empty( $value->title ) ? $value->title : '<em>' . __('Untitle Button', 'leadgeneration')	. '</em>';
					$data[] = array(							
					'ID' => $value->id,	
		...
		public function list_count() {
			global $wpdb;
			$data = $this->data;
			$resultat = $wpdb->get_results("SELECT * FROM " . $data . " WHERE tool='buttons' order by id asc");
