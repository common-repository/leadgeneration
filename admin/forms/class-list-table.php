	...
	*/
	class LG_List_Table extends WP_List_Table{
		...
		*/
		...
		{
			if ($column_name === 'mails'){
				$url = admin_url( '/admin.php?page=leadgeneration-mails&s=' . urlencode( $item['ID'] ) );
				$value   = '<a href="' . $url . '">' . $item['mails'] . '</a>'; 			
			}			
			else {
		...
		*/
		private function table_data()
		{
			...
			if ($resultat) {				
				foreach ($resultat as $key => $value) {		
					$option_name = '_lg_tool_form_view_counter_' . $value->id;
					$tool_view = get_option( $option_name, '0' );	
					$table_mails = $wpdb->prefix . 'lg_mails';
					$mails_count = intval( $wpdb->get_var( "SELECT COUNT(ID) FROM " . $table_mails . " WHERE form_id='" . $value->id ."'" ) );
					if ( !empty($tool_view) ) {
						$ñonversion = round( $mails_count/$tool_view*100 ) . '%';
					}
					else {
						$ñonversion = 0;
					}					
					$title = !empty( $value->title ) ? $value->title : '<em>' . __('Untitle Form', 'leadgeneration')	. '</em>';
					$data[] = array(
					'ID' => $value->id,	
		...
		public function list_count() {
			global $wpdb;
			$data = $this->data;
			$resultat = $wpdb->get_results("SELECT * FROM " . $data . " WHERE tool='forms' order by id asc");
